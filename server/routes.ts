...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/feedback", upload.any(), async (req, res) => {
    try {
      ...
      // Send to webhook - try local test first if in development
      try {
        let webhookResponse;
        ...
        if (process.env.NODE_ENV === 'development') {
          ...
          webhookResponse = await fetch('http://localhost:5000/api/test-webhook', {
            method: 'POST',
            body: webhookData,
          });
          
          if (webhookResponse.ok) {
            console.log('Local test successful, sending to production...');
          }
        }
